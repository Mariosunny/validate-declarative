...
Intuitive, lightweight, and highly extensible.
...
## Overview
**validate-declarative** is a utility that takes a schema and some data and determines if the data matches the structure described by the schema. A **schema** is a plain Javascript object with some special properties.

```javascript
// a schema that describes a tweet from Twitter
const tweetSchema = {
  $test: function(object) {
...
```
Keys in the schema beginning with `$` are constraints. Constraints define the rules for validating data. The most common constraint is the `$test` constraint, which defines a type test.

In the example above, `$test` defines a type that is a *string* and also has a *length* of 24 characters or less. Any object that does not pass these two conditions is considered an *invalid type*.

Thus, the following tweet satisfies the schema:
```javascript
let myTweet1 = "Hello world!";
```

But neither of these satisfy the schema:
```javascript
let myTweet2 = 5;
let myTweet3 = "Lorem ipsum dolor sit amet, consectetur adipiscing.";
```

To validate data against the schema, use `verify()`.
...
You can create a schema for any Javascript object, including multi-dimensional arrays and deeply nested objects. See
...
Same as `verify()`, but returns an array of error objects (see Errors) describing each constraint failure in detail. If the data satisfies the schema, the array will be empty, otherwise the array will be non-empty.

## Errors
This section describes the four errors that could be generated by `validate()`.
...
## Constraints
Constraints define the rules for validating data. They are embedded in schema objects alongside ordinary properties. Constraints begin with `$` to differentiate them from ordinary properties.
...
#### Validating a property using a regular expression
...
## Built-in Types
There are several basic types included in this package. Their definitions are listed below.
